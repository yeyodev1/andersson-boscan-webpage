  ...
  <Teleport to="body">
    <Transition name="modal-fade">
      <div v-if="isOpen" class="lm-overlay" @click.self="tryClose">
        ...
        <!-- ── CONFIRM CLOSE DIALOG (floats above everything) ── -->
        <Transition name="modal-fade">
          <div v-if="showConfirm" class="lm-confirm-overlay" @click.self="showConfirm = false">
            <div class="lm-confirm">
              ...
              <div class="lm-confirm-btns">
                <button class="lm-btn-ghost" @click="showConfirm = false">
                  Seguir llenando
                </button>
                <button class="lm-btn-danger" @click="forceClose">
                  Sí, cerrar
                </button>
              </div>
            </div>
        ...
        <div class="lm-card-wrapper">
          <!-- Close floats outside the scrollable card -->
          ...
          <Transition name="slide-up" mode="out-in">
            <div v-if="step === 1" key="s1" class="lm-body">
              ...
              <div class="lm-grid">
                ...
                <input v-model="form.celular"  type="tel"   class="lm-input lm-input--full" placeholder="WhatsApp / celular (con código de país)" />
              </div>
              ...
              <button class="lm-btn-primary" @click="goToStep2">
...
// Check if the user has entered any data worth protecting
const hasData = computed(() =>
  form.value.nombre || form.value.apellido || form.value.correo ||
  form.value.celular || form.value.tipo_cliente || form.value.objetivo ||
...
function goToStep2() {
  step1Error.value = ''
  const f = form.value
  if (!f.nombre || !f.apellido || !f.correo || !f.celular) {
...
async function handleSubmit() {
  ...
  const webhookUrl = import.meta.env.VITE_GHL_WEBHOOK_URL as string
...
.lm-card-wrapper {
  ...
  width: 100%;
  max-width: 540px;
  margin-top: 20px; /* space so the floating X is visible */
}
...
// ── CONFIRM OVERLAY (above everything, blurs the card) ────────
.lm-confirm-overlay {
  position: fixed;
  inset: 0;
  z-index: 2100;
