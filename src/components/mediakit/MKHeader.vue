      ...
      <nav class="mkh__menu">
        ...
        >
          ...
          <span class="mkh__menu-label">
            {{ item.label }}
            <span v-if="item.action === 'scroll' && route.path !== '/publicidad'" class="mkh__menu-hint">↗ Home</span>
          </span>
...
const navItems = [
  { num: '01', label: 'Inicio',        action: 'home',   target: '/publicidad' },
...
function isActive(item: typeof navItems[number]) {
  if (item.action === 'home') return route.path === '/publicidad'
  if (item.action === 'route') return route.path === item.target
  return false
}

function goHome() {
  if (route.path !== '/publicidad') router.push('/publicidad')
...
function handleNav(item: typeof navItems[number]) {
  ...
  setTimeout(() => {
    if (item.action === 'home') {
      ...
      if (route.path !== '/publicidad') {
        router.push('/publicidad').then(() => setTimeout(() => scrollTo(item.target), 300))
      } else {
